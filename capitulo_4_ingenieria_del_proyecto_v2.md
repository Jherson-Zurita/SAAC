...
En este capítulo se detalla la especificación técnica, el diseño de arquitectura de software, la implementación de componentes, la representación de diagramas y el conjunto de pruebas que constituyen el sistema **SAAC v2.0** (*Software Architecture Analysis Companion*). Toda la descripción técnica aquí expuesta se encuentra estrictamente respaldada y verificada contra el código fuente del repositorio del proyecto.
...
***Figura 4.3. Captura de la Interfaz Principal del IDE SAAC v2.0 (GraphForge UI).*** *Fuente: Elaboración propia.*

![](./capturas/saac_graphforge_ui.png)

***Figura 4.4. Esquema de Disposición del Layout GraphForge UI en 5 Regiones.*** *Fuente: Elaboración propia.*
...
***Figura 4.5. Vista Previa de Diagrama C4 en el Canvas de SAAC v2.0.*** *Fuente: Elaboración propia.*
...
***Figura 4.6. Mapa de Calor de Acoplamiento (Coupling Matrix Heatmap).*** *Fuente: Elaboración propia.*

![](./capturas/coupling_heatmap_preview.png)

* **Grafo de Fuerza con Nodos Proporcionales (`ForceGraphView`)**:
  * **Qué representa**: Representación de red basada en física de fuerza (algoritmo Dagre/D3) donde el tamaño del área de cada nodo es directamente proporcional a su acoplamiento total ($Ca + Ce$).
  * **Cómo interpretarlo**: Los nodos de gran tamaño con múltiples aristas concéntricas identifican módulos centrales con alto riesgo de impacto (*God Modules* o puntos únicos de fallo).
* **Mapa de Calor de Acoplamiento (`CouplingHeatmapView`)**:
  * **Qué representa**: Matriz de adyacencia cuadrada $N \times N$ de módulos.
  * **Cómo interpretarlo**: Las celdas resaltadas en tinte rojo oscuro (`#EF6B73`) señalan acoplamientos eferentes directos. Filas con muchas celdas rojas indican módulos altamente dependientes ($Ce \gg 0$).
* **Treemap de Mantenibilidad (`TreemapView`)**:
  * **Qué representa**: Diagrama de bloques donde el área de cada rectángulo equivale a sus Líneas de Código (LOC) y el color refleja el Índice de Mantenibilidad.
  * **Cómo interpretarlo**: Rectángulos de gran tamaño coloreados en rojo (`#EF6B73`, mantenibilidad $<50$) o amarillo (`#E7B85B`, mantenibilidad $<70$) identifican la deuda técnica prioritaria para refactorización.
* **Mapa de Propiedad y Bus Factor (`OwnershipMapView`)**:
  * **Qué representa**: Distribución de autoría por módulo y alertas de *Bus Factor*.
  * **Cómo interpretarlo**: Destaca módulos complejos ($LOC > 150, Ce > 8$) desarrollados por un único autor, advirtiendo sobre el riesgo de pérdida de conocimiento organizacional.
* **Línea de Tiempo Histórica (`TimelineView`)**:
  ...
  * **Cómo interpretarlo**: Permite medir si el *Fitness Score* del proyecto mejora o degrada tras cada *commit* o *sprint* de desarrollo.
...
***Figura 4.7. Chat del Asistente de IA con Renderizado de Markdown y Resaltado de Sintaxis.*** *Fuente: Elaboración propia.*
...
***Figura 4.8. Diagrama de Flujo del Mecanismo de Fallback del Cliente de IA.*** *Fuente: Elaboración propia.*
...
5. **Capturas e Imágenes**: Se generó el archivo de vista previa interactiva `ui_preview_standalone.html` y las imágenes de demostración almacenadas en la carpeta `capturas/`.
